/*
 * prof.c: version 1.2 of 1/19/83
 * 
 * eliminated use of floating point
 */
# ifdef SCCS
static char *sccsid = "@(#)prof.c	1.2 (NSC) 1/19/83";
# endif
...
int	totime;			/* total time for all routines */
int	maxtime;		/* maximum time of any routine (for plot) */
...
printprof()
{
	int perctime, perctenths, actime;

	actime = 0;
	printf(" %%time  cumtics  #call  ts/call  name\n");
	if (!lflg)
	...
	for (np = nl; np<npe-1; np++) {
		if (zflg == 0 && np->time == 0 && np->ncall == 0)
			continue;
		/*time = np->time/totime;*/
		perctime = (np->time*1000)/totime;
		perctenths = perctime % 10;
		perctime /= 10;
		actime += np->time;
		printf("%4d.%01d%9d", perctime, perctenths, actime);
		if (np->ncall != 0)
			printf("%7ld %8d",
			    np->ncall, np->time/(np->ncall));
		else
			printf("%7.7s %8.8s", "", "");
		if (dflag == 0) printf("  %s\n", np->name);
		else printf("  %s		%d\n",np->name, np->value);
...
getsymtab()
{
	...
	for (i = xbuf.a_syms; i > 0; i -= sizeof(struct nlist)) {
		...
		if ((nbuf.n_etype > N_METN) || (nbuf.n_addrmode != A_PC))
			continue;
		if (aflg==0 && nbuf.n_ext==0)
			continue;
		nname++;
	}
	if (nname == 0) {
	...
	/* read in the mod table */
	fseek(nfile, N_TXTOFF(xbuf), 0);
	modtab = (char *)calloc(xbuf.a_mod,1);
	if (modtab == 0) {
		fprintf(stderr, "prof: No room for %d bytes of mod table\n",
		     xbuf.a_mod);
		done();
	}
	fread(modtab, xbuf.a_mod, 1, nfile);
	...
	nname = 0;
	for (i = xbuf.a_syms; i > 0; i -= sizeof(struct nlist)) {
		struct modtable *curmod;
		struct nlist nbuf;
		fread(&nbuf, sizeof(nbuf), 1, nfile);
		if (N_MODULE(nbuf)) {
		   curmod = (struct modtable *) (modtab+nbuf.n_value);
		   continue;
		}
		if (nbuf.n_etype>N_METN || nbuf.n_addrmode!=A_PC)
			continue;
		if (aflg==0 && nbuf.n_ext==0)
			continue;
		npe->value = (curmod->mt_pb+nbuf.n_value)/sizeof(UNIT);
		npe->name = strtab+nbuf.n_namestr;
		npe++;
		nname++;
	}
	npe->value = -1;
...
asgnsamples()
{
	...
	for (i=0; i < nsamples; i++) {
		...
		/*pcl = lowpc + scale*i;*/
		j = k*i;
		pcl = lowpc + j/nsamples;
		/*pch = lowpc + scale*(i+1);*/
		pch = lowpc + (j+k)/nsamples;
		time = ccnt;
		totime += time;
		if(time > maxtime)
			maxtime = time;
		for (j=0; j<nname; j++) {
			...
			if (overlap>0)
				nl[j].time += (overlap*time+(overlap>>1))/
						(pch-pcl); /* roundoff */
		}
	}
	if (totime==0) {
