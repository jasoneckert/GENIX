/*
 * dumpdir.c: version 1.3 of 12/6/82
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)dumpdir.c	1.3 (NSC) 12/6/82";
# endif
...
char *argv[];
{
	...
	tbf = (struct tbuf *)valloc(NTREC*BSIZE);
	mktemp(dirfile);
...
char *b;
{
	...
	if (bct >= NTREC) {
		for (i = 0; i < NTREC; i++)
			((struct spcl *)&tbf->a[i*BSIZE])->c_magic = 0;
		bct = 0;
		if ((i = read(mt, tbf->a, NTREC*BSIZE)) < 0)
			quit();
		if (i == 0) {
			bct = NTREC + 1;
			volno++;
