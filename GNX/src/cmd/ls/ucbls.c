/*
 * ucbls.c: version 1.6 of 10/18/83
 * Unix System Command Source File
 */
# ifdef SCCS
static char *sccsid = "@(#)ucbls.c	1.6 (NSC) 10/18/83";
# endif
 ...
 * Modified by Landon Noll (chongo /\../\) Sun Oct 16 22:44:50 PDT 1983
 *	will always produce a username on the -l option
 *
...
struct	utmp	utmp;
#define NMAX	8	/* allow only 8 chars in a name so that the names
			 * table can be indexed by a shift right 3 bits
			 */
	...
	if (lflg) {
		...
		if(gflg) {
			...
			if (nflg == 0 && (name = getname(p->luid))) {
				/* WARNING: dont expand the format
				 * beyond 8 chars unless you alter
				 * the names table.  there user names
				 * are packed strings without NULL
				 * terminators
				 */
				printf("%-8.8s", name);
			}
...
#define NUID	32768	/* maximum uid-1  we will handle in memory */
/* STARTING must be 0 due to static variables 0 filled before runtime */
#define STARTING 0	/* the name table is not in memory */
#define NOMEM 1		/* we dont have enough memory for a name table */
#define TABLE_BUILT 2	/* the in memory table is now complete */
#define TABLE_STARTING 3	/* table just formed, do a setpwent first */
#define TABLE_GROWING 4		/* we did a setpwent, table is growing */

/*
 * getname - get athe user name from a uder-id (uid) quickly
 *
 * getname attempts to build a fast access name table which
 * is used to return names for options like: ls -l
 *
 * returns: pointer to the first 8 chars, or 0 if not found
 *	    the name string may not be NULL terminated
 *
 * NOTE: if for some reason getname can not form the in memory
 *       table, getname will return pointers to the names found
 *	 because of this, the name returned may not be available
 *	 on the next call of getname.
 */

char *
getname( uid )

int uid;	/* the user id we want to find */

{
	register struct passwd *pw;	/* the entry read from passwd */
	register long int uidshift;	/* uid converted into a names index */
	static init;		/* state of the routine */
	struct passwd *getpwent();
	static char namefound[NMAX];	/* used to return name if NOMEM */
	static int uidfound;		/* used to return name if NOMEM */
	static char *names;	/* table if user names we have seen so far
				 * records should be a power of 2 in length
				 * to allow for fast indexing
				 */
	
	uidshift = uid << 3;	/* convert uid to a names index */

	/* be sure the name table is in memory */
	if ( init == STARTING ) {
		names = (char *)sbrk( NMAX*NUID );	/* try to get space */
		if ( (int)names == -1 ) {
			init = NOMEM;	/* we dont have table memory *sigh* */
			namefound[0] = 0;	/* no last name found */
			uidfound = -1;		/* no last name found */
		} else {
			init = TABLE_STARTING;	/* the table is just starting */
		}
	}

	/* quicky if no memory table and we have the same previous uid */
	if ( init == NOMEM && uid == uidfound  && uid != -1 ) {
		return( namefound );	/* return the last name found */
	}

	/* if the uid is in range, and we have the name in memory table, done */
	if ( (uid >= 0 && uid < NUID) && (init != NOMEM) && names[uidshift]) {
		return (&names[uidshift]);	/* return the name */
	}

	/* the uid isnt in the table  table complete => we dont know the name */
	if (init == TABLE_BUILT && (uid >= 0 && uid < NUID)) {
		return (0);	/* who? */
	}

	/* start the password fetching system if need be */
	if ( init == TABLE_STARTING || init == NOMEM ) {
		setpwent();	/* start the password record fetching system */
		if ( init == TABLE_STARTING ) {
			init = TABLE_GROWING;	/* the memory table can grow */
		}

	/* if the uid is out of table range, we must research the list */
	} else if ( uid < 0 || uid >= NUID ) {
		if ( init == TABLE_GROWING ) {
			endpwent();	/* end the password fetching system */
		}
		setpwent();	/* start the password record fetching system */
	}

	/* search the password file till done or we find the uid */
	while (pw = getpwent()) {
		/* if we dont have memory, or the uid read if out of range */
		if ((init == NOMEM) || (pw->pw_uid < 0 || pw->pw_uid >= NUID)) {
			/* if the uid is correct return the name */
			if ( uid == pw->pw_uid ) {
				/* keep the name in static area */
				uidfound = pw->pw_uid;
				strncpy( namefound, pw->pw_name, NMAX );
				/* end the password fetching system if no table */
				if ( init == NOMEM ) {
					endpwent();
				}
				return( namefound );	/* found it */
			}

		/* we can use the memory table */
		} else {
			/* store the name in the names table if need be */
			if ( names[ ((long)(pw->pw_uid))<<3 ] == 0 ) {
				/* store the name */
				strncpy( &(names[ ((long)(pw->pw_uid))<<3 ]),
				  pw->pw_name, NMAX );
			}
		
			/* if we found our name, return it */
			if (pw->pw_uid == uid) {
				return (&(names[uidshift]));	/* found it */
			}
		}
	}

	/* we finished the table now */
	if ( init != NOMEM ) {	/* if we had a table, it is now built */
		init = TABLE_BUILT;
	}
	endpwent();	/* end the password fetching system */
	return (0);	/* nothing found, sorry */
}
#endif
