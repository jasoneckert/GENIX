/*
 * dodelt.c: version 1.2 of 11/15/82
 * (System III) Sccs Source
 */
# ifdef SCCS
static char *sccsid = "@(#)dodelt.c	1.2 (NSC) 11/15/82";
# endif
...
char type;
{
	...
	while (getadel(pkt,&dt) == BDELTAB) {
		if (pkt->p_idel == 0) {
			...
			pkt->p_idel  = malloc (n = ((dt.d_serial + 1) * sizeof(*pkt->p_idel)));
			zero(pkt->p_idel,n);
			pkt->p_apply = malloc (n = ((dt.d_serial + 1) * sizeof(*pkt->p_apply)));
			zero(pkt->p_apply,n);
...
struct ixg *ixgp;
{
	...
	svip = v = ip = malloc (BUFSIZ);	/* rewrote to avoid alloca */
	++p;
	type = *p++;
	NONBLANK(p);
	while (numeric(*p)) {
	...
	for (cur = ixgp; cur = (prev = cur)->i_next; )
		;
	prev->i_next = cur = malloc (sizeof(*cur) + (cnt - 1) * sizeof(cur->i_ser[0]));
	cur->i_next = 0;
	cur->i_type = type;
	cur->i_cnt = cnt;
	move(v,cur->i_ser,cnt * sizeof(cur->i_ser[0]));

	/* Explicitly free storage that was formerly implicitly freed. */
	free (svip);
}
