/*
 * date_ab.c: version 1.3 of 10/17/83
 * (System III) Sccs Source
 */
# ifdef SCCS
static char *sccsid = "@(#)date_ab.c	1.3 (NSC) 10/17/83";
# endif
...
static long	eleven_pm =  60*60*23;
...
long *bdt;
{
	...
	/* Figure out GMT correction: */
	hour_diff  = gmtime    (&eleven_pm)->tm_hour;
	hour_diff -= localtime (&eleven_pm)->tm_hour;
	tim       += (60*60) * hour_diff;
...
/*
 * Grab the next two chars of the date string
 * and return their integer value.  Return -1
 * if we've run out of date string.
 */
g2()
{
	register int c;
