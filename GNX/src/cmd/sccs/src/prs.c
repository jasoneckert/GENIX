/*
 * prs.c: version 1.3 of 11/6/82
 * (System III) Sccs Source
 */
# ifdef SCCS
static char *sccsid = "@(#)prs.c	1.3 (NSC) 11/6/82";
# endif
...
register char *p;
{
	if (PWindex(p,":C:") != -1)	/* check for Comment keyword */
		HAD_CM = 1;
	if (PWindex(p,":MR:") != -1)	/* check for MR keyword */
		HAD_MR = 1;
	if (PWindex(p,":UN:") != -1)	/* check for User name keyword */
		HAD_UN = 1;
	if (PWindex(p,":FD:") != -1)	/* check for descriptive text kyword */
		HAD_FD = 1;
	if (PWindex(p,":BD:") != -1)	/* check for body keyword */
		HAD_BD = 1;
}
