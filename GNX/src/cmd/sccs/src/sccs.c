/*
 * sccs.c: version 1.5 of 10/17/83
 * Sccs Interface Program
 */
# ifdef SCCS
static char *sccsid = "@(#)sccs.c	1.5 (NSC) 10/17/83";
# endif
...
# ifndef PROGPATH
# define PROGPATH(name)	"/usr/nsc/name"	/* place to find binaries */
# endif PROGPATH
...
struct sccsprog SccsProg[] =
{
	...
	/*
	 * The following are System III sccs additions.
	 * All but val and prs seem hardly worth it given
	 * some of the hooks already in this program.
	 */
	"prs",		PROG,	0,			PROGPATH(prs),
	"val",		PROG,	0,			PROGPATH(val),
	"vc",		PROG,	0,			PROGPATH(vc),
	...
	char **argv;
{
	...
	*/
	...
	while ((dp = readdir (dirfd)) != NULL)
	{
		...
		strncpy(basefile, &dp->d_name[2], dp->d_namlen - 2);
		basefile[dp->d_namlen - 2] = '\0';
		...
		*/
		...
		{
			strncpy(buf, &dp->d_name[2], dp->d_namlen - 2);
			buf[dp->d_namlen - 2] = '\0';
			unlink(buf);
		}
	}
