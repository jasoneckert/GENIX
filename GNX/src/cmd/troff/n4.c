/*
 * n4.c: version 1.5 of 5/10/83
 * Unix System Command Source File
 */
# ifdef SCCS
static char *sccsid = "@(#)n4.c	1.5 (NSC) 5/10/83";
# endif
...
static	int	nrfmtbuf[MAXWD+1];
/* Support for \g feature ala system 3 used by the mm macro package.  This
   routine decodes the internal format generated by the previous routine.
*/
setaf()		/* Return the format of a number register - in the .ap form */
{
	register int i, j;
	register int *p;

	if ((i=findr(getsn())) == -1) return; /* no register */
	p = nrfmtbuf;
	if (fmt[i] > MAXWD)	/* A, a, I, or i.  Assumes checks above. */
		*p++ = (fmt[i] & BMASK);
	else {
	...
	*p = 0;
	/* Push the decoded form back onto the input stream. */
	if (cp) {
		if (ap) {
			prstrfl("\\g: too much pushback into the input stream\n");
			return;
		} else
			ap = cp;
	}
