/*
 * expreserve.c: version 1.3 of 6/16/83
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)expreserve.c	1.3 (NSC) 6/16/83";
# endif
 ...
 * to preserve the entire contents of /usr/tmp (root only).
 *
	...
	int argc;
{
	...
	/*
	 * ... else preserve all the stuff in /usr/tmp, removing
	 * it as we go.
	 */
	if (chdir("/usr/tmp") < 0) {
		perror("/usr/tmp");
