/*
 * stty.c: version 1.6 of 8/31/83
 * Unix System Command Source File
 */
# ifdef SCCS
static char *sccsid = "@(#)stty.c	1.6 (NSC) 8/31/83";
# endif
...
char	**iargv;
{
	...
	special[0].cp = &mode.sg_erase;
	special[1].cp = &mode.sg_kill;
	special[2].cp = &tc.t_intrc;
	special[3].cp = &tc.t_quitc;
	special[4].cp = &tc.t_startc;
	special[5].cp = &tc.t_stopc;
	special[6].cp = &tc.t_eofc;
	special[7].cp = &tc.t_brkc;
	special[8].cp = &ltc.t_suspc;
	special[9].cp = &ltc.t_dsuspc;
	special[10].cp = &ltc.t_rprntc;
	special[11].cp = &ltc.t_flushc;
	special[12].cp = &ltc.t_werasc;
	special[13].cp = &ltc.t_lnextc;
	special[14].cp = &ltc.t_infoc;
...
{
	int i;
	char xc;	/* temp for compiler bug */

	if(!arg)
		return(0);
	i = 0;
loop:
	/*if(arg[i] != string[i])	compiler bug */
		xc = string[i];
		if (arg[i] != xc)
		return(0);
	if(arg[i++] != '\0')
