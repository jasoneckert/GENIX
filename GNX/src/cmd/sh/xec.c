/*
 * xec.c: version 1.3 of 9/9/82
 * Unix System Command Source File
 */
# ifdef SCCS
static char *sccsid = "@(#)xec.c	1.3 (NSC) 9/9/82";
# endif
...
*/
		...
		case TFORK:
			...
			ELSE	/* this is the forked branch (child) of execute */
				...
				initio(t->treio);
				IF type!=TCOM
				THEN	execute(t->forktre,1,0,0); /* FIX */
		...
		case TPAR:
			rename(dup(2),output);
			execute(t->partre,execflg,0,0); /* FIX */
		...
		case TLST:
			execute(t->lstlef,0,0,0); /* FIX */
			/* execute(t->lstlef,0); */
			execute(t->lstrit,execflg,0,0); /* FIX */
			/* execute(t->lstrit,execflg); */
			break;

		case TAND:
			IF execute(t->lstlef,0,0,0)==0 /* FIX */
			/* IF execute(t->lstlef,0)==0 */
			THEN	execute(t->lstrit,execflg,0,0); /* FIX */
		...
		case TUN:
			BEGIN
			   ...
			   loopcnt++;
			   WHILE execbrk==0 ANDF (execute(t->whtre,0,0,0)==0)==(type==TWH) /* FIX */
			   /* WHILE execbrk==0 ANDF (execute(t->whtre,0)==0)==(type==TWH) */
			   DO i=execute(t->dotre,0,0,0); /* FIX */
		...
		case TIF:
			IF execute(t->iftre,0,0,0)==0  /* FIX */
			THEN	execute(t->thtre,execflg,0,0);  /* FIX */
			ELSE	execute(t->eltre,execflg,0,0);  /* FIX */
			...
			BEGIN
			   ...
			   DO	ARGPTR		rex=t->regptr;
				...
				DO	REG STRING	s;
					...
					THEN	execute(t->regcom,0,0,0); /* FIX */
	...
	UFD		f;
{
	...
	THEN	initf(f);
	FI
	execute(cmd(NL, NLFLG|MTFLG),0,0,0); /* FIX */
