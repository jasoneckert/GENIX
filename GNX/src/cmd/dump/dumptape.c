/*
 * dumptape.c: version 1.4 of 3/30/83
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)dumptape.c	1.4 (NSC) 3/30/83";
# endif
...
/*
	...
	for(i=0; i<BSIZE; i++)
		tblock->a[trecno][i] = *dp++;
*/
	fastcp(tblock->a[trecno], dp, BSIZE);
	tdaddr[trecno] = 0;
	trecno++;
	spcl.c_tapea++;
	if(trecno >= NTREC)
...
loop:
	...
	if(d != 0) {
		bread(d, tblock->a[si], BSIZE);
		tdaddr[si] = 0;
		goto loop;
	}
	trecno = 0;
	if (write(to, tblock->a[0], sizeof(tblock->a)) != sizeof(tblock->a) ){
...
#ifdef MONITOR
	monitor(-1); /* Cause mon.out filename to include process id. */
#endif MONITOR
	exit(status);
}
