/*
 * dump.h: version 1.3 of 12/7/82
 * 
 *
 * @(#)dump.h	1.3	(National Semiconductor)	12/7/82
 */
...
#define	TAPE	"/dev/rtc0"		/* default tape device */
#define	DISK	"/dev/rdc1g"		/* default disk */
...
/*	Declare page aligned buffers	*/
struct 	tblk	{
	char	a[NTREC][BSIZE];
} 	*tblock;
#define	ITABSIZE (BSIZE*NI)
struct	itbl	{
	struct	dinode	tab[INOPB * NI];
}	*itab;
char	*blockba;
