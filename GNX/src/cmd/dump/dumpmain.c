/*
 * dumpmain.c: version 1.3 of 11/12/82
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)dumpmain.c	1.3 (NSC) 11/12/82";
# endif
...
*/
	...
	/*	allocate page-aligned buffer for tape */
	tblock = (struct tblk *) valloc(sizeof(struct tblk));
	/*	allocate page-aligned buffer large enough for largest table we
	 *	read.
	 */
	itab = (struct itbl *) valloc(ITABSIZE);
	blockba = (char *) valloc(BSIZE);
...
*/
	msg("estimated %ld tape blocks on %d tape(s).\n", esize, etapes);
