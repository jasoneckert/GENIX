/*
 * dumpoptr.c: version 1.2 of 11/9/82
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)dumpoptr.c	1.2 (NSC) 11/9/82";
# endif
...
*/
		deltat = tstart_writing - tnow +
			 ((((tnow - tstart_writing)*1000) / blockswritten )
			 * esize ) / 1000;
		msg("%3d.%-d%% done, finished in %d:%02d\n",
			(blockswritten*100)/esize,
			(((blockswritten*100)%esize) *100) / esize,
			deltat/3600, (deltat%3600)/60);
	}
}
