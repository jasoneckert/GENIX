/*
 * fsck.c: version 1.9 of 8/18/83
 * Unix System Command Source File
 */
# ifdef SCCS
static char *sccsid = "@(#)fsck.c	1.9 (NSC) 8/18/83";
# endif
...
#define CYLSIZE		200	/* default cyl size for spacing */
#define MAXCYL		614	/* maximum cylinder size */
...
#define MAXRAW	10		/* largest raw read (in blks) */
...
#define	DUPTBLSIZE	25	/* num of dup blocks to remember */
...
char	*devname;
char	iobuffer[(MAXRAW+1) * BSIZE];		/* buffer for doing raw */
char	*iobufptr;				/* page boundary in iobuffer */
...
char	*argv[];
{
	...
	sync();
	iobufptr = (char *) (((int)iobuffer + PGOFSET) &~ PGOFSET);
	while(--argc > 0 && **++argv == '-') {
		switch(*++*argv) {
			case 'p':
				preen++;
...
char *buf;
{
	if(lseek(fcp->rfdes,blk<<BSHIFT,0) < 0)
		...
		if (read(fcp->rfdes, iobufptr, size) == size) {
			movebuf(buf, iobufptr, size);
			return(YES);
		}
	}
	rwerr("READ",blk);
...
char *buf;
{
	...
	if(lseek(fcp->wfdes,blk<<BSHIFT,0) < 0)
		...
		movebuf(iobufptr, buf, size);
		if (write(fcp->wfdes, iobufptr, size) == size) {
			fcp->mod = 1;
			return(YES);
		}
	}
