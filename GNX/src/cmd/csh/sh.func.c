/*
 * sh.func.c: version 3.5 of 2/7/83
 * Csh with job control
 */
# ifdef SCCS
static char *sccsid = "@(#)sh.func.c	3.5 (NSC) 2/7/83";
# endif
...
static struct limits {
	...
	LIM_NORAISE,	"noraise",	1,	"",		0,	0,
	LIM_CPU,	"cputime",	1,	"seconds",	0,	0,
	LIM_FSIZE,	"filesize",	1024,	"kbytes",	0,	0,
	LIM_DATA,	"datasize",	1024,	"kbytes",	0,	0,
	LIM_STACK,	"stacksize",	1024,	"kbytes",	0,	0,
	LIM_CORE,	"coredumpsize",	1024,	"kbytes",	0,	0,
	LIM_MAXRSS,	"memoryuse",	1024,	"kbytes",	0,	0,
	-1,		0,
};

static	char	noraise = 0;

/* Here when csh is initialized, to set the initial limit values */
liminit()
{
	register struct limits *lp;

	for (lp = limits; lp->limconst >= 0; lp++) {
		lp->limval = vlimit(lp->limconst, -1);
		if ((lp->limconst == LIM_NORAISE) && lp->limval && getuid()) {
			noraise = 1;
		}
	}
}
	...
	register char **v;
{
	...
	if (*v == 0) {
		for (lp = limits+1; lp->limconst >= 0; lp++)
			plim(lp);
		if (noraise) printf("Limits cannot be raised\n");
		return;
	}
	lp = findlim(v[0]);
	if (v[1] == 0) {
...
#ifdef notdef				/* floating point version of getval */
static
getval(lp, v)
	register struct limits *lp;
	char **v;
...
#else				/* integer version of getval */
static
getval(lp, v)
	register struct limits *lp;
	char **v;
...
unsetlim(lp)
	register struct limits *lp;
{
	if (noraise && lp->limconst == LIM_NORAISE) {
		error("Can't unlimit noraise");
	}
	lp->limset = 0;
	lp->limval = vlimit(lp->limconst, -1);
	if ((!noraise) && (lp->limconst < INFINITY)) {
		lp->limset = 1;
		lp->limval = INFINITY;
	}
}
...
setlim(lp, limit)
	register struct limits *lp;
{
	if (noraise && ((limit > vlimit(lp->limconst, -1)) ||
	                ((lp->limconst == LIM_NORAISE) && (limit == 0)))) {
		error("Cannot raise limit");
	}
	else {
		lp->limset = 1;
		lp->limval = limit;
...
/* Here from a forked child to set the limits for the new process */
setlimits()
{
	register struct limits *lp;

	for (lp = limits; lp->limconst >= 0; lp++) {
		if (lp->limset) (void)vlimit(lp->limconst, lp->limval);
	}
}
