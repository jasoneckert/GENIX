...
/*
 * cgram.y: version 5.1 of 9/8/83
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)cgram.y	5.1 (NSC) 9/8/83";
# endif
/* JZ 4/14/83:  Changed the RETURN code to check and see if skip_return_code is
		set and if it is to branch to struc_skip_lab instead of retlab.
		That is set by setstr if this is structure returning function
		that is returning the value of another call.  This allows
		only one copy to be made down to the bottom of the tree.*/
/* JZ 4/23/83:  added bregvar to the places where regvar and autooff are saved
		in psavbc. */
%}
...
statement:	   e   SM
		...
		|  RETURN e  SM
			={  register NODE *temp;
			    extern struc_skip_lab,skip_return_code;
			    idname = curftn;
			    ...
			    retstat |= RETVAL;
			    /* In the case of a structure returning function
			       which is returning the value returned by a
			       called function.  The return code that
			       copies the structure must be skipped since
			       that is being done by the called function.
			       */
			    if (skip_return_code)
			    {
			        branch(struc_skip_lab);
				skip_return_code=0; /* reset global switch */
			    }
			    else branch( retlab );
			    reached = 0;
			    }
