...
/*
 * pftn.c: version 5.1 of 9/8/83
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)pftn.c	5.1 (NSC) 9/8/83";
# endif

/* jima 7/26/82: INIT fix from Jay. */
/* JZ   4/23/83: Added bregvar and minbvar for B register variables to defid */
/* JZ   4/23/83: Changed fixclass to understand about floating registers */
/* JZ   5/6/83:  fincode now takes the NODE instead of the constant value*/
# include "mfile1"
...
# endif
	...
	case MOE:
		...
		psave( idp );
		break;
	case REGISTER: /* JZ added DOUBLE register variables */
	        if (type == DOUBLE) p->offset = bregvar--;
		if (bregvar < minbvar) minbvar = bregvar ;
		else p->offset = regvar--;
...
/* 	The following is being removed since it removes the UNARY AND
/*	   which is needed JZ 7-21-82
...
# ifndef RTOLBYTES
	...
	if( p->sclass != REGISTER ){ 
		/* in case we are allocating stack space for register args */
...
fixclass( class, type ) TWORD type; {
	...
	case REGISTER:
		...
		else if((( regvar >= MINRVAR && type != FLOAT &&type != DOUBLE)
		        ||((type == FLOAT || type == DOUBLE)&& bregvar >= MINBVAR))
