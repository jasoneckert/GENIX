/*
 * pk0.c: version 1.1 of 5/4/83
 * Uucp Source
 */
# ifdef SCCS
static char *sccsid = "@(#)pk0.c	1.1 (NSC) 5/4/83";
# endif
...
unsigned short sum;
	...
	PKDEBUG(9, "pkaccept: %s", "made it out of above loop\n");
	pk->p_imap = 0;
	UNLOCK;
...
free:
	...
	*/
	...
	pk->p_rcount = accept;
	PKDEBUG(9, "pkaccept: pk->p_msg = %o\n", pk->p_msg);
	PKDEBUG(9, "pkaccept: accept = %o\n", accept);
	return(accept);
}
...
char *cp, **bp;
	...
	xfr = 0;
	count = 0;
	while (pkaccept(pk)==0) PKDEBUG(9, "calling", " pkaccept\n");
	PKDEBUG(5, "returned from pkaccept", "\n");
