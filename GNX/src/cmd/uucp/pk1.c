/*
 * pk1.c: version 1.1 of 5/4/83
 * Uucp Source
 */
# ifdef SCCS
static char *sccsid = "@(#)pk1.c	1.1 (NSC) 5/4/83";
# endif
 ...
 *
 *	01/06/81  PAG	Added more debugging
 *
 */
#define USER	1
#include <stdio.h>
...
struct pack *ipk;
{
	...
	PKDEBUG(7, "receive %s", "");
	PKDEBUG(7, "s = %o, ", (unsigned) p[0]);
	PKDEBUG(7, "k = %o, ", (unsigned) p[1]);
	PKDEBUG(7, "x = %o, ", (unsigned) p[2]);
	PKDEBUG(7, "y = %o, ", (unsigned) p[3]);
	PKDEBUG(7, "c = %o, ", (unsigned) p[4]);
	PKDEBUG(7, "z = %o\n", (unsigned) p[5]);
	p += 2;
	sum = (unsigned) *p++ & 0377;
	sum |= (unsigned) *p << 8;
