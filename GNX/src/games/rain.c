static	char *sccsid = "@(#)rain.c	1.4 (National Semiconductor) 7/31/82";
#include <stdio.h>
#include <sgtty.h>
#include <signal.h>
...
char *argv[];
{
    ...
    if ((LI=tgetnum("li",&tcp))==-1) LI=24;
    if ((CO=tgetnum("co",&tcp))==-1) CO=80;
    TE=tgetstr("te",&tcp);
    TI=tgetstr("ti",&tcp);
    UP=tgetstr("up",&tcp);
    if (!(LL=tgetstr("ll",&tcp))) strcpy(LL=malloc(10),tgoto(CM,0,LI-1));
    gtty(1, &sg);
    ospeed=sg.sg_ospeed;
