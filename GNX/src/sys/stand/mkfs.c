/*
 * mkfs.c: version 1.10 of 9/26/83
 * Unix System Command Source File
 */
# ifdef SCCS
static char *sccsid = "@(#)mkfs.c	1.10 (NSC) 9/26/83";
# endif
...
#endif
{
	...
	fin = NULL;
	{
		...
		disksize = fdisksize(fso);
		printf("file sys size (enter <cr> for default size of %d): ",
			disksize);
		gets(protos);
		proto = protos;
	}
...
char *bf;
{
	...
	n = read(fsi, iobufptr, BSIZE);
	if(n != BSIZE) {
		printf("read error: %ld\n", bno);
		exit(1);
	} else
		movebuf(bf, iobufptr, BSIZE);
}
...
char *bf;
{
	...
	movebuf(iobufptr, bf, BSIZE);
	lseek(fso, bno*BSIZE, 0);
	n = write(fso, iobufptr, BSIZE);
	if(n != BSIZE) {
