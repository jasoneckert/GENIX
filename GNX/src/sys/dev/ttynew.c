/*
 * ttynew.c: version 3.9 of 8/31/83
 * 
 */
# ifdef SCCS
static char *sccsid = "@(#)ttynew.c	3.9 (NSC) 8/31/83";
# endif
...
loop:
	...
	if (tp->t_flags&RAW) {
		...
		while (tp->t_rawq.c_cc && passc(getc(&tp->t_rawq))>=0)
			;
/*
 * Don't return here, but fall through in case TBLOCK is set.
 * Otherwise TANDEM won't work in raw mode.
		return (0);
 */
	} else {
...
register struct tty *tp;
{
	...
	if ((tp->t_state&CARR_ON)==0)
		return (NULL);
loop:
	while (up->u_procp->p_pgrp != tp->t_pgrp && tp == up->u_ttyp &&
	    (tp->t_local&LTOSTOP) && 
	    up->u_signal[SIGTTOU] != SIG_IGN &&
	    up->u_signal[SIGTTOU] != SIG_HOLD &&
	    (up->u_procp->p_flag&SDETACH)==0) {
		gsignal(up->u_procp->p_pgrp, SIGTTOU);
		sleep((caddr_t)&lbolt, TTIPRI);
	}
	while (up->u_count) {
		cc = MIN(up->u_count, OBUFSIZ);
		cp = obuf;
		...
		while (cc) {
			if (tp->t_flags&RAW || tp->t_local&LLITOUT)
				...
				ce=0;
				while(((lpartab[*(unsigned char *)(cp+ce)&0177]&077)==0)&&(ce<cc))
					ce++;
				if (ce==0) {
					tp->t_rocount = 0;
